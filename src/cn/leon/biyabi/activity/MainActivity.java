...
import cn.leon.biyabi.db.InviteMessgeDao;
import cn.leon.biyabi.db.EMUserDao;
import cn.leon.biyabi.domain.EMUser;
import cn.leon.biyabi.domain.InviteMessage;
import cn.leon.biyabi.fragment.ChatAllHistoryFragment;
import cn.leon.biyabi.fragment.ContactlistFragment;
import cn.leon.biyabi.fragment.SettingsFragment;
...
public class MainActivity extends BaseActivity implements EMEventListener {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(cn.leon.biyabi.R.layout.activity_main);
		mContext = this;
		initView();
		...
		// 添加显示第一个fragment
		getSupportFragmentManager().beginTransaction().add(cn.leon.biyabi.R.id.fragment_container, chatHistoryFragment)
				.add(cn.leon.biyabi.R.id.fragment_container, contactListFragment).hide(contactListFragment).show(chatHistoryFragment)
				.commit();
	...
	static void asyncFetchContactsFromServer(){
	    HXSDKHelper.getInstance().asyncFetchContactsFromServer(new EMValueCallBack<List<String>>(){
            ...
            public void onSuccess(List<String> usernames) {
                ...
                String strChat = context.getString(cn.leon.biyabi.R.string.Application_and_notify);
                newFriends.setNick(strChat);
        
                userlist.put(Constant.NEW_FRIENDS_USERNAME, newFriends);
                // 添加"群聊"
                EMUser groupUser = new EMUser();
                String strGroup = context.getString(cn.leon.biyabi.R.string.group_chat);
...
//                String strChatRoom = context.getString(cn.leon.superwechat.R.string.chat_room);
...
//        		String strRobot = context.getString(cn.leon.superwechat.R.string.robot_chat);
...
//        		userlist.put(Constant.CHAT_ROBOT, robotUser);
	...
	private void initView() {
		unreadLabel = (TextView) findViewById(cn.leon.biyabi.R.id.unread_msg_number);
		unreadAddressLable = (TextView) findViewById(cn.leon.biyabi.R.id.unread_address_number);
		mTabs = new Button[3];
		mTabs[0] = (Button) findViewById(cn.leon.biyabi.R.id.btn_conversation);
		mTabs[1] = (Button) findViewById(cn.leon.biyabi.R.id.btn_address_list);
		mTabs[2] = (Button) findViewById(cn.leon.biyabi.R.id.btn_setting);
		// 把第一个tab设为选中状态
	...
	public void onTabClicked(View view) {
		switch (view.getId()) {
		case cn.leon.biyabi.R.id.btn_conversation:
			index = 0;
			break;
		case cn.leon.biyabi.R.id.btn_address_list:
			index = 1;
			break;
		case cn.leon.biyabi.R.id.btn_setting:
			index = 2;
			break;
		}
		if (currentTabIndex != index) {
			...
			if (!fragments[index].isAdded()) {
				trx.add(cn.leon.biyabi.R.id.fragment_container, fragments[index]);
			}
...
//					unreadAddressLable.setText(String.valueOf(count));
	...
	public class MyContactListener implements EMContactListener {
				...
				if (isAdd) {
					try {
						String path = new ApiParams()
                                .with(I.Contact.USER_NAME, BiyabiApplication.getInstance().getUserName())
		...
		public void onContactDeleted(final List<String> usernameList) {
			...
			runOnUiThread(new Runnable() {
				public void run() {
					// 如果正在与此用户的聊天页面
					String st10 = getResources().getString(cn.leon.biyabi.R.string.have_you_removed);
		...
		return new Response.Listener<Contact>() {
			...
			public void onResponse(Contact contact) {
				if (contact != null && contact.isResult()) {
					ArrayList<Contact> contactList = BiyabiApplication.getInstance().getContactList();
					HashMap<String, Contact> userList = BiyabiApplication.getInstance().getUserList();
					contactList.add(contact);
					userList.put(contact.getMContactCname(), contact);
					mContext.sendStickyBroadcast(new Intent("update_contact_list"));
					Utils.showToast(mContext, R.string.Add_buddy_success, Toast.LENGTH_LONG);
				} else {
	...
	public class MyConnectionListener implements EMConnectionListener {
		...
		public void onDisconnected(final int error) {
			final String st1 = getResources().getString(cn.leon.biyabi.R.string.can_not_connect_chat_server_connection);
			final String st2 = getResources().getString(cn.leon.biyabi.R.string.the_current_network);
			runOnUiThread(new Runnable() {
	...
	public class MyGroupChangeListener implements EMGroupChangeListener {
		...
		public void onInvitationReceived(String groupId, String groupName, String inviter, String reason) {
			...
			// 被邀请
			String st3 = getResources().getString(cn.leon.biyabi.R.string.Invite_you_to_join_a_group_chat);
		...
		public void onApplicationAccept(String groupId, String groupName, String accepter) {
			...
			String st4 = getResources().getString(cn.leon.biyabi.R.string.Agreed_to_your_group_chat_application);
			// 加群申请被同意
			EMMessage msg = EMMessage.createReceiveMessage(Type.TXT);
			msg.setChatType(ChatType.GroupChat);
	...
	private void showConflictDialog() {
		...
		String st = getResources().getString(cn.leon.biyabi.R.string.Logoff_notification);
		if (!MainActivity.this.isFinishing()) {
			...
			try {
				...
				conflictBuilder.setMessage(cn.leon.biyabi.R.string.connect_conflict);
				conflictBuilder.setPositiveButton(cn.leon.biyabi.R.string.ok, new DialogInterface.OnClickListener() {
	...
	private void showAccountRemovedDialog() {
		...
		DemoHXSDKHelper.getInstance().logout(true,null);
		String st5 = getResources().getString(cn.leon.biyabi.R.string.Remove_the_notification);
		if (!MainActivity.this.isFinishing()) {
			// clear up global variables
			try {
				...
				accountRemovedBuilder.setMessage(cn.leon.biyabi.R.string.em_user_remove);
				accountRemovedBuilder.setPositiveButton(cn.leon.biyabi.R.string.ok, new DialogInterface.OnClickListener() {
