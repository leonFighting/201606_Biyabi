...
public class PublicChatRoomsActivity extends BaseActivity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(cn.leon.biyabi.R.layout.activity_public_groups);

		// 搜索框
		searching = (EditText)findViewById(cn.leon.biyabi.R.id.query);
		clean = (ImageButton)findViewById(cn.leon.biyabi.R.id.search_clear);
		searching.setHint(cn.leon.biyabi.R.string.search);
		inputMethodManager = (InputMethodManager)getSystemService(Context.INPUT_METHOD_SERVICE);
		
		pb = (ProgressBar) findViewById(cn.leon.biyabi.R.id.progressBar);
		listView = (ListView) findViewById(cn.leon.biyabi.R.id.list);
		title = (TextView)findViewById(cn.leon.biyabi.R.id.tv_title);
		title.setText(getResources().getString(cn.leon.biyabi.R.string.chat_room));
		chatRoomList = new ArrayList<EMChatRoom>();
		rooms = new ArrayList<EMChatRoom>();
		
		View footView = getLayoutInflater().inflate(cn.leon.biyabi.R.layout.listview_footer_view, null);
        footLoadingLayout = (LinearLayout) footView.findViewById(cn.leon.biyabi.R.id.loading_layout);
        footLoadingPB = (ProgressBar)footView.findViewById(cn.leon.biyabi.R.id.loading_bar);
        footLoadingText = (TextView) footView.findViewById(cn.leon.biyabi.R.id.loading_text);
...
//                                    footLoadingLayout.setVisibility(View.VISIBLE);
                            ...
                            if(isFirstLoading){
                                ...
                                if(chatRooms.size() < pagesize){
                                    ...
                                    footLoadingText.setText(getResources().getString(cn.leon.biyabi.R.string.no_more_messages));
                                }
                                adapter.notifyDataSetChanged();
                            }
                            isLoading = false;
                    ...
                    runOnUiThread(new Runnable() {
                        public void run() {
                            ...
                            pb.setVisibility(View.INVISIBLE);
                            footLoadingLayout.setVisibility(View.GONE);
                            Toast.makeText(PublicChatRoomsActivity.this, getResources().getString(cn.leon.biyabi.R.string.failed_to_load_data), 0).show();
                        }
                    });
                }
            }
	...
	private class ChatRoomAdapter extends ArrayAdapter<EMChatRoom> {
		...
		public View getView(int position, View convertView, ViewGroup parent) {
			...
			((TextView) convertView.findViewById(cn.leon.biyabi.R.id.name)).setText(getItem(position).getName());
