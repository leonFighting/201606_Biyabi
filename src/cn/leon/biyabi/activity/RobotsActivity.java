...
public class RobotsActivity extends BaseActivity {
	...
	protected void onCreate(Bundle arg0) {
		super.onCreate(arg0);
		setContentView(cn.leon.biyabi.R.layout.fragment_robots);
		inputMethodManager = (InputMethodManager) getSystemService(INPUT_METHOD_SERVICE);
		mListView = (ListView) findViewById(cn.leon.biyabi.R.id.list);
		swipeRefreshLayout = (SwipeRefreshLayout) findViewById(cn.leon.biyabi.R.id.swipe_layout);
		swipeRefreshLayout.setColorSchemeResources(android.R.color.holo_blue_bright, android.R.color.holo_green_light,
				android.R.color.holo_orange_light, android.R.color.holo_red_light);
		progressBar = findViewById(cn.leon.biyabi.R.id.progress_bar);
	...
	class RobotAdapter extends ArrayAdapter<RobotUser> {
		...
		public View getView(int position, View convertView, ViewGroup parent) {
			if (convertView == null) {
				convertView = inflater.inflate(cn.leon.biyabi.R.layout.row_robots, null);
			}
			((TextView) convertView.findViewById(cn.leon.biyabi.R.id.name)).setText(getItem(position).getNick());
			return convertView;
		}
