 ...
 */
package cn.leon.biyabi.fragment;
...
import cn.leon.biyabi.activity.AddContactActivity;
import cn.leon.biyabi.activity.ChatActivity;
import cn.leon.biyabi.activity.GroupsActivity;
import cn.leon.biyabi.activity.MainActivity;
import cn.leon.biyabi.activity.NewFriendsMsgActivity;
import cn.leon.biyabi.activity.PublicChatRoomsActivity;
import cn.leon.biyabi.activity.RobotsActivity;
import cn.leon.biyabi.applib.controller.HXSDKHelper;
import cn.leon.biyabi.applib.controller.HXSDKHelper.HXSyncListener;
import com.easemob.chat.EMContactManager;
import cn.leon.biyabi.Constant;
import cn.leon.biyabi.DemoHXSDKHelper;
import cn.leon.biyabi.adapter.ContactAdapter;
...
import cn.leon.biyabi.widget.Sidebar;
...
public class ContactlistFragment extends Fragment {
				    ...
				    getActivity().runOnUiThread(new Runnable(){
		                ...
		                public void run() {
		                    if(success){
		                        ...
		                        String s1 = getResources().getString(cn.leon.biyabi.R.string.get_failed_please_check);
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
		return inflater.inflate(cn.leon.biyabi.R.layout.fragment_contact_list, container, false);
	}
	...
	public void onActivityCreated(Bundle savedInstanceState) {
		...
		listView = (ListView) getView().findViewById(cn.leon.biyabi.R.id.list);
		sidebar = (Sidebar) getView().findViewById(cn.leon.biyabi.R.id.sidebar);
		...
		//搜索框
		query = (EditText) getView().findViewById(cn.leon.biyabi.R.id.query);
		query.setHint(cn.leon.biyabi.R.string.search);
		clearSearch = (ImageButton) getView().findViewById(cn.leon.biyabi.R.id.search_clear);
		...
		// 设置adapter
		adapter = new ContactAdapter(getActivity(), cn.leon.biyabi.R.layout.row_contact, mContactist);
		listView.setAdapter(adapter);
		setListener();

		progressBar = (View) getView().findViewById(cn.leon.biyabi.R.id.progress_bar);
	...
	private void setAddContactListener() {
		ImageView addContactView = (ImageView) getView().findViewById(cn.leon.biyabi.R.id.iv_new_contact);
		// 进入添加好友页
	...
	public void onCreateContextMenu(ContextMenu menu, View v, ContextMenuInfo menuInfo) {
		...
		if (((AdapterContextMenuInfo) menuInfo).position > 1) {
			...
			getActivity().getMenuInflater().inflate(cn.leon.biyabi.R.menu.context_contact_list, menu);
		}
	}

	@Override
	public boolean onContextItemSelected(MenuItem item) {
		if (item.getItemId() == cn.leon.biyabi.R.id.delete_contact) {
			try {
                // 删除此联系人
			...
			return true;
		}else if(item.getItemId() == cn.leon.biyabi.R.id.add_to_blacklist){
	...
	public void deleteContact(final Contact tobeDeleteUser) {
		String st1 = getResources().getString(cn.leon.biyabi.R.string.deleting);
		final String st2 = getResources().getString(cn.leon.biyabi.R.string.Delete_failed);
	...
	private void moveToBlacklist(final String username){
		final ProgressDialog pd = new ProgressDialog(getActivity());
		String st1 = getResources().getString(cn.leon.biyabi.R.string.Is_moved_into_blacklist);
		final String st2 = getResources().getString(cn.leon.biyabi.R.string.Move_into_blacklist_success);
		final String st3 = getResources().getString(cn.leon.biyabi.R.string.Move_into_blacklist_failure);
		pd.setMessage(st1);
		pd.setCanceledOnTouchOutside(false);
		pd.show();
		new Thread(new Runnable() {
	...
	private void getContactList() {
		...
		String strGroup = getActivity().getString(cn.leon.biyabi.R.string.group_chat);
		...
		String strChat = getActivity().getString(cn.leon.biyabi.R.string.Application_and_notify);
